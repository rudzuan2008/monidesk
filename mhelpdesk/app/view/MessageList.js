Ext.define('mhelpdesk.view.MessageList', {
	...
	config : {
		...
		itemTpl : new Ext.XTemplate(
				...
				'	<img style="float: left;" src="images/user_comment.png?s=32&d=mm" />',
				'	<p class="triangle-right left"><span class="nickname">Me ({created:date("d-m-Y H:i")}) :</span> <br>',
				'	<span style="font-weight: normal;">{message} &nbsp;<br>',
				'	<tpl if="this.hasAttachment(attachments) == true">',
				'		<span style="color:green;cursor: pointer;text-align: right;"><a href="{[this.getDefaultServer()]}mattachment.php?id={ticket_id}&ref={hash}"><i accessKey="{ticket_id}" class="fa fa-paperclip fa-lg"></i>&nbsp;{[this.getLocaleText(values.locale,"Attachment")]}</a></span><br>',
				'	</tpl></span>',
				'	</p>',
				'<tpl else>',
				'	<img style="position: absolute; top: 10px; right: 10px;" src="images/user_comment.png?s=32&d=mm" />',
				'	<p class="triangle-right right"><span class="nickname">{staff_name} ({created:date("d-m-Y H:i")}) :</span> <br>',
				'	<span style="font-weight: normal;">{message} &nbsp;<br>',
				'	<tpl if="this.hasAttachment(attachments) == true">',
				'		<span style="color:green;cursor: pointer;text-align: right;"><a href="{[this.getDefaultServer()]}mattachment.php?id={ticket_id}&ref={hash}"><i accessKey="{ticket_id}" class="fa fa-paperclip fa-lg"></i>&nbsp;{[this.getLocaleText(values.locale,"Attachment")]}</a></span><br>',
				'	</tpl></span>', '	</p>', '</tpl>', 
