		...
		control : {
			...
			mainPage2 : {
				...
				tapVisitor: function(thisPage, thisButton, e, eOpts) {
					...
					var clientId= system.getLastId();
					me.getTicketByVisitor('ticket.client_id='+clientId+' AND ticket.status="open"',true, 
						function(records,operation,success) {
							console.error(records.length);
							var visitorResidentPage = Ext.create('mhelpdesk.view.VisitorResidentPage');	
							me.pushPage(visitorResidentPage);
						}
					)
				}
				...
				save : function( thisPage, thisButton, e, eOpts) {
					...
					if (record) {
						...
						if (isValid) {
							...
							var topic_id = 4;
							me.doActionTicket(client_id,subject,message,visitor,contactNo,vehicleNo,pass,topic_id,null,null,
			...
			registerVisitorResident : {
				...
				save : function( thisPage, thisButton, e, eOpts) {
					...
					var unitNo = fields['asset_unit'];
					var dateVisit = fields['dateVisit'];
					var visitor = fields['name'];
					var contactNo = fields['phone'];
					var vehicleNo = fields['vehicle'];
					var timeVisit = fields['timeVisit'];
					
					var localAllClientStore = Ext.getStore('localAllClientStore');
					var record = utils.findRecord(localAllClientStore,'client_id',system.getLastId());
					if (record) {
						var isValid = true;
						var errors = new Array();
						if (!visitor) {
						...
						if (!dateVisit) {
							isValid = false;
							errors.push({
								field : locale.getText('Visit Date'),
								reason : locale.getText("fieldEmpty")
							});
						}
						if (!timeVisit) {
							isValid = false;
							errors.push({
								field : locale.getText('Visit Time'),
								reason : locale.getText("fieldEmpty")
							});
						}
						if (!vehicleNo) {
						...
						if (isValid) {
							var client_id = record.data.client_id;
							var subject = "New Visitor on Unit "+record.data.asset_unit+" ("+record.data.fullname+")";							
							var message = "Visitor Name:"+visitor+", Vehicle:"+vehicleNo+" (Pre-Register)";
							var topic_id = 4;
							if (category=="delivery") {
								subject = "New Delivery on Unit "+record.data.asset_unit+" ("+record.data.fullname+")";							
								message = "Delivery Name:"+visitor+", Vehicle:"+vehicleNo+" (Pre-Register)";
								topic_id = 5;
							}
							console.error(dateVisit+' '+timeVisit);
							
							me.doActionTicket(client_id,subject,message,visitor,contactNo,vehicleNo,null,topic_id,dateVisit,timeVisit,
							function(msg, success){
								if (success) {
									var clientId= system.getLastId();
									me.getTicketByVisitor('ticket.client_id='+clientId+' AND ticket.status="open"',true, 
			...
			visitorResidentPage: {
				addnew : function(page, thisButton, e, eOpts) {
					var me = this;
					var locMod = page.getItemId()+'.addnew';
					app.dh('Start','...',locMod);
					me.load_product(system.getLastId(), false,
						function(records,operation,success) {
							...
							var registerVisitorResident = Ext.create('mhelpdesk.view.visitor.RegisterVisitorResident');	
							me.pushPage(registerVisitorResident);
							//if (record) registerVisitorResident.down('#asset_unit').setValue(record.data.serial_no);
						}
					);
				},
				close: function(page, thisButton, e, eOpts) {
				...
				filter : function(page, thisButton, e, eOpts) {
					...
					if (selection=="status") {
						var status = page.down('#filterStatus').getValue();
						var clientId= system.getLastId();
						me.getTicketByVisitor("ticket.client_id="+clientId+ " AND ticket.status='"+status+"'",true, function(records,operation,success){
							
						});
					}else if (selection=="created") {
						var dateCreated = page.down('#filterText').getValue();
						var clientId= system.getLastId();
						me.getTicketByVisitor("ticket.client_id="+clientId+ " AND ticket.created LIKE '"+dateCreated+"%'",true, function(records,operation,success){
			...
			visitorResidentList : {
				itemtap : function(thisList, index, target, record, e, eOpts) {
					...
					try{
						...
						console.error(topic_id);
						if (topic_id==5) {
							page = Ext.create('mhelpdesk.view.visitor.ViewDeliveryResident',{
								ticketId:ticket_id
							});
						}else{	
							page = Ext.create('mhelpdesk.view.visitor.ViewVisitorResident',{
								ticketId:ticket_id
							});
						}
...
//							});
		...
		try {
			...
			var mainApp = function(proceed){
				if (proceed) {
					...
					me.debugHandler(locMod, "log", "clientId", system.getLastId());
	...
	doActionTicket : function(client_id, subject, message, visitor,contactNo,vehicleNo,pass,topic_id,date_create,time_create, callback) {
		var me = this;
		...
		function doSubmit(start, timeout, callback) {
			...
			if (date_create && time_create) {
				created = utils.dbShortDate(date_create)+' '+ utils.formatTime('%l:%M:%S',time_create);
			}
			Ext.Ajax.request({
				useDefaultXhrHeader : false,
				timeout : timeout,
	...
	getTicketByVisitor : function(filter, refresh, callback) {
		...
		try {
			...
			var params = {
				...
				table : 'rz_ticket',
				filter : strFilter,
				sort: '[{"property":"ticket.created","direction":"ASC"}]'
			};
