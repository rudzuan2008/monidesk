...
// 		$date = date_create($receive_date);
// 		$fulldate = date_format($date,'Y-m-d');// '"F j, Y, g:i a"');
function db_date($var) {
	$date = date_create($var);
	return date_format($date,'Y-m-d');// '"F j, Y, g:i a"');
}
function db_datetime($var) {
	$date = date_create($var);
	return date_format($date,'Y-m-d H:i:s');// '"F j, Y, g:i a"');
}
...
// 		if (isset($dblink)) {
// 			echo "here";
// 			$dblink=db_connect(DBHOST,DBUSER,DBPASS,DBNAME);
// 		}
