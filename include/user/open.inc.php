...
<form action="open.php" method="POST" enctype="multipart/form-data">
    ...
    if($cfg->getUserLogRequired()) {
    	if ($thisuser && ($organization=$thisuser->getOrganization())) {
                ?>
	    <div class="input">
	    	 <span class="label"><?= _('Organization:')?></span>&nbsp;<?=$organization ?>
	    </div>
    	<?php } 
    	if ($thisuser && ($product=$thisuser->getProduct())) {
                ?>	
        <div class="input">
	    	 <span class="label"><?= _('Product:')?></span>&nbsp;<?=$product ?>
	    </div>
	    <?php }         
    } ?>
    <div class="input">
    ...
    <div class="input">
        ...
        <span>
        	<?php if ($thisuser && ($phone=$thisuser->getPhone())) {
                ?>
                <input type="hidden" name="phone" size="30" value="<?=$phone?>"><?=$phone?>
            <?php }else { ?> 
                <input type="text" name="phone" size="30" maxlength="28" value="<?=$info['phone']?>">
            <?php } ?>
