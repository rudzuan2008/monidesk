...
<form class="rz-form" action="admin.php" method="POST" name="role">
  ...
  <table style="width:100%; border: 0px solid;" class="tform">
      ...
      <tr>
          ...
          <?php }else{?>
          	<?= Sys::getCompany() ?><input type="hidden" name="company_id" value="<?=$info['company_id']?>">
          <?php }?>
          &nbsp;<font class="error">*&nbsp;<?=$errors['company_id']?></font></td>
      </tr>
      <tr><th><?= _('Role Name:') ?></th>
          <td><input type="text" name="role_name" size="55" value="<?=$info['role_name']?>">
      ...
      <tr><th><br><?= _('Dept Access') ?></th>
          <td class="mainTableAlt"><i><?= _('Select departments role members are allowed to access in addition to their own department.') ?></i>
              ...
              if ($thisuser->getUserName()=="admin") {
              	...
              	$depts= db_query('SELECT dept_id,dept_name FROM '.DEPT_TABLE.' WHERE company_id='. $info['company_id'] . ' ORDER BY dept_name');
              }
  ...
  <div class="centered">
    ...
    <input class="button" type="button" name="cancel" value="<?= _('Cancel') ?>" onClick='window.location.href="admin.php?t=roles&menu=<?=$pmenu?>&sid=<?=$info['company_id']?>"'>
  </div>
</form>