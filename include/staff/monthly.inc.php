...
$qwhere =" WHERE DATE_FORMAT(ticket.created,'%m-%Y')='$current_month-$current_year'";
...
$pageNav->setURL('report.php?t=monthly',$qstr.'&sort='.urlencode($_REQUEST['sort']).'&order='.urlencode($_REQUEST['order']));
...
<input type="hidden" name="t" value="monthly">
<table width="100%">
    <tr><td>
<div>
    <?= _('Selection:') ?>
    <?= _('Month') ?>&nbsp;
     <select name="month">
            <option value='01' <?= $current_month == '01' ?'selected':''?> ><?= _('JAN') ?></option>
            <option value='02' <?= $current_month == '02' ?'selected':''?> ><?= _('FEB') ?></option>
            <option value='03' <?= $current_month == '03' ?'selected':''?> ><?= _('MAR') ?></option>
            <option value='04' <?= $current_month == '04' ?'selected':''?> ><?= _('APR') ?></option>
            <option value='05' <?= $current_month == '05' ?'selected':''?> ><?= _('MAY') ?></option>
            <option value='06' <?= $current_month == '06' ?'selected':''?> ><?= _('JUN') ?></option>
            <option value='07' <?= $current_month == '07' ?'selected':''?> ><?= _('JUL') ?></option>
            <option value='08' <?= $current_month == '08' ?'selected':''?> ><?= _('AUG') ?></option>
            <option value='09' <?= $current_month == '09' ?'selected':''?> ><?= _('SEP') ?></option>
            <option value='10' <?= $current_month == '10' ?'selected':''?> ><?= _('OCT') ?></option>
            <option value='11' <?= $current_month == '11' ?'selected':''?> ><?= _('NOV') ?></option>
            <option value='12' <?= $current_month == '12' ?'selected':''?> ><?= _('DEC') ?></option>           
    </select>
...
<div class="msg"><?= _('Monthly Reports') ?></div>
<div id="divToPrint">
<form action="report.php?t=monthly" method="post">
    ...
    <table border="1" cellspacing=0 cellpadding=2 class="dtable" align="center" width="100%">
      <tr style="background-color: #ADADAD;">
          ...
          <th width="270"><a href="report.php?t=monthly&startDate=<?=$current_date?>&sort=subject&order=<?= $order == 'DESC' ?'ASC':'DESC'?>"><?= _('Subject') ?></a></th>
          <th width="170"><?= _('From') ?></th>
          <th width=200><a href="report.php?t=monthly&sort=dept&order=<?= $order == 'DESC' ?'ASC':'DESC'?>"><?= _('Department') ?></a></th>
...
<script type="text/javascript">
      ...
      function drawChart() {
        ...
        // Set chart options
        var options = {
           	title:'Percentage By Status for $current_month/$current_year',
